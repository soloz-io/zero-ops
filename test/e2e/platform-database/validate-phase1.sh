...
# Get platform-db pod
POD=$(kubectl get pod -n hub-platform-data -l cnpg.io/cluster=platform-db,role=primary -o jsonpath='{.items[0].metadata.name}')
...
kubectl get secret control-plane-db-credentials -n hub-platform-data &>/dev/null
if [ $? -eq 0 ]; then
    echo "✓ control-plane-db-credentials secret exists"
    DB_URL=$(kubectl get secret control-plane-db-credentials -n hub-platform-data -o jsonpath='{.data.url}' | base64 -d)
...
kubectl get secret hub-db-credentials -n hub-platform-data &>/dev/null
if [ $? -eq 0 ]; then
    echo "✓ hub-db-credentials secret exists"
    DB_URL=$(kubectl get secret hub-db-credentials -n hub-platform-data -o jsonpath='{.data.url}' | base64 -d)
    if [[ "$DB_URL" == *"hub"* ]]; then
...
kubectl exec -n hub-platform-data $POD -- psql -U postgres -d control_plane -c "
...
kubectl exec -n hub-platform-data $POD -- psql -U postgres -d hub -c "
