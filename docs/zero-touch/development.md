...
- can you review the below critic and provide your feedback?
...
- i want you to first focus on creating a requirement spec for #### Journey A: Platform Bootstrap (Platform Admin) only. the plan is to build in phases one by one. Sicne I follow TDD, i want you to provide me the test cases that need to be first created for this Journey A, Management Cluster ("Mothership").. i will ask the team to create requirements based on the test cases you provide and use that as accpetance critetia. provide a comprehensive list of high level test cases, not codes, for the team to build the requirements and design for the test cases to pass. Keep the test cases behavior based and e2e flow. Do not include unit testing or integration testing. Only focus on e2e testing cases.
