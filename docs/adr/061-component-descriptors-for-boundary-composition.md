...
**Environment-parameterised components remain enumerated.** A descriptor states a fixed source. Three components in boundary 03 do not have one: their path is a function of the environment slug, the provider, or the topology, which is precisely the directory-based promotion ADR-037 establishes. Their source is not a property of the component but of the environment the boundary is being rendered for, and a static descriptor cannot express it.
...
**Sync waves are unchanged in policy.** ADR-021's restriction stands: waves order CRDs before the operators that own them within boundary 01 and are not used to order application workloads. A descriptor records the wave a component already has; it does not extend where waves may be used. Waves must be unique within a boundary, so that ordering is a property of the declaration rather than of the order elements happen to appear.
...
A component's declaration, content and values occupy one directory, so a change to one component is a change to one place and is reviewable in isolation.
