...
2. **Internal Exponential Backoff:** All custom Go applications (`kube-sbt`, `mcp-server`, `auth-proxy`) MUST implement internal, infinite exponential backoff loops for their external dependencies (Databases, NATS, APIs) rather than executing `log.Fatal()` or `os.Exit(1)` on startup.
3. **Probe Separation:** 
