...
```text
Hub Operator reconciles SpokePool
       ↓
Hub Operator creates Certificate CR (72h TTL, cert-manager.io/v1)
       ↓
cert-manager + infisical-issuer fulfill the Certificate
       ↓
Certificate → Kubernetes Secret (argocd-agent-<spoke>-tls)
       ↓
Crossplane wraps Secret in ClusterResourceSet Object MR
       ↓
ClusterResourceSet delivers Secret to Spoke
       ↓
ArgoCD Agent starts with mTLS
       ↓
Spoke cert-manager takes over certificate lifecycle
```

**Bootstrap Certificate Private Key:** The private key is generated by cert-manager on the Hub and stored in the Kubernetes Secret. It is delivered to the Spoke via ClusterResourceSet. After delivery, cert-manager on the Spoke manages all certificate lifecycle. Private key material must never be handled by Hub Operator code.
...
- Reconcile existence (drift detection).
...
- Creates `Certificate` CRs for SpokePool bootstrap certificates (declarative intent).
- Creates `SpokeMachineIdentity` CRs declaring desired Spoke identity state.
- Creates `ClusterResourceSet` payloads for delivery.

The Hub Operator must never:
