...
### A rule of thumb: 
#### CLI change → 
./scripts/dev/local-e2e.sh 0.1.16-rc.24 clean cli scaffold bootstrap
./scripts/dev/local-e2e.sh 0.1.16-rc.24 cli bootstrap
#### manifest change → 
DOMAIN=nutgraf.in make e2e-fresh VERSION=0.1.16-rc.24
make e2e-fresh VERSION=0.1.16-rc.24    # clean → publish → cli → scaffold → bootstrap
make e2e-clean VERSION=0.1.16-rc.24    # tear the last run down and stop
make e2e       VERSION=0.1.16-rc.24    # as before — cannot reach clean

#### PROVIDER=hybrid
DOMAIN=nutgraf.in PROVIDER=hybrid REGION=fsn1 make e2e-fresh VERSION=0.1.16-rc.25
DOMAIN=nutgraf.in PROVIDER=hybrid make e2e-fresh VERSION=0.1.16-rc.24
...
make publish-local VERSION=0.1.16-rc.24
2. Promote the running cluster — one edit, both fields, exactly as Renovate would:
cd .local-e2e/acme-gitops
sed -i '' 's/0\.1\.16-rc\.3/0.1.16-rc.24/g' clusters/acme-hub/bundle.yaml
git commit -am "promote acme-hub to 0.1.16-rc.24" && git push
cd ../..
3. Resume the bootstrap once it's Synced, to finish adr045-commit and what follows:
./scripts/dev/local-e2e.sh 0.1.16-rc.24 bootstrap
