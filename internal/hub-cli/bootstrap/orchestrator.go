...
func (o *Orchestrator) deployPlatform(ctx context.Context, kubeconfig string) error {
	...
	// Bootstrap the platform-db-ca via cert-manager SelfSigned issuer.
	// This breaks the circular dependency: cert-manager → Issuer → Infisical → platform-db-ca → cert-manager.
	// The SelfSigned issuer requires no external PKI backend and is scoped as a bootstrap primitive only
	// (must not be used for tenant, Spoke, service, ingress, or fleet certificates — infisical-issuer handles those).
	if err := o.ensurePlatformCACertificate(ctx, kubeconfig); err != nil {
		return fmt.Errorf("platform-db-ca certificate not ready: %w", err)
	}
...
// ensurePlatformCACertificate bootstraps the Hub-local platform-db-ca via
// cert-manager's built-in SelfSigned ClusterIssuer. This is a bootstrap
// primitive that breaks the circular dependency between cert-manager, the
// PKI issuer backend, and the database CA.
//
// The SelfSigned issuer must not be used for tenant, Spoke, service,
// ingress, or fleet certificates (infisical-issuer handles those once
// Infisical is bootstrapped).
func (o *Orchestrator) ensurePlatformCACertificate(ctx context.Context, kubeconfig string) error {
	fmt.Println("[platform-deploy] Bootstrapping platform-db-ca via cert-manager SelfSigned issuer...")

	// 1. Create SelfSigned ClusterIssuer
	issuerYAML := `apiVersion: cert-manager.io/v1
kind: ClusterIssuer
metadata:
  name: platform-bootstrap-selfsigned
  labels:
    platform.nutgraf.in/scope: bootstrap
    platform.nutgraf.in/bootstrapped-by: hub-cli
spec:
  selfSigned: {}`

	applyIssuer := exec.CommandContext(ctx, "kubectl", "--kubeconfig", kubeconfig, "apply", "-f", "-")
	applyIssuer.Stdin = strings.NewReader(issuerYAML)
	if out, err := applyIssuer.CombinedOutput(); err != nil {
		return fmt.Errorf("failed to create SelfSigned ClusterIssuer: %w\n%s", err, out)
	}
	fmt.Println("[platform-deploy] ✓ SelfSigned ClusterIssuer created")

	// 2. Create platform-db-ca Certificate CR
...
spec:
  ...
  issuerRef:
	...
	applyCert := exec.CommandContext(ctx, "kubectl", "--kubeconfig", kubeconfig, "apply", "-f", "-")
	applyCert.Stdin = strings.NewReader(certYAML)
	if out, err := applyCert.CombinedOutput(); err != nil {
		return fmt.Errorf("failed to create platform-db-ca Certificate: %w\n%s", err, out)
	}
	fmt.Println("[platform-deploy] ✓ platform-db-ca Certificate created")

	// 3. Wait for cert-manager to issue the certificate and create the Secret
	fmt.Println("[platform-deploy] Waiting for platform-db-ca Secret...")
	waitCmd := exec.CommandContext(ctx, "kubectl", "--kubeconfig", kubeconfig,
		"wait", "certificate", "platform-db-ca",
		"-n", "platform-data",
		"--for=condition=Ready",
		"--timeout=2m",
	)
	if out, err := waitCmd.CombinedOutput(); err != nil {
		return fmt.Errorf("platform-db-ca Certificate not ready: %w\n%s", err, out)
	}
	fmt.Println("[platform-deploy] ✓ platform-db-ca Secret ready")
