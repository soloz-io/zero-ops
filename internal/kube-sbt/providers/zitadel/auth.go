...
func (a *Auth) CreateUser(ctx context.Context, user models.User) (*models.User, error) {
	...
	if user.Password != "" {
		...
		//
		// It is generated by the caller (tenant.go: generateInitialPassword) and
		// filed in the tenant's secret store. Nobody types it twice, nobody
		// remembers it, and nobody outside the platform ever chose it. With
		// changeRequired false — as this read until 2026-09-26 — that random
		// string IS the account's password forever: ZITADEL never prompts, so the
		// owner's only route in is for someone to read a secret out of Infisical
		// on their behalf, and the account accumulates no
