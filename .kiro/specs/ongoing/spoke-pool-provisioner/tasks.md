...
- [x] 1.8.3 Issue ArgoCD Principal server certificate
  - Run: `argocd-agentctl pki issue principal --principal-context hub-cluster --dns "argocd-agent-principal.platform-ops.svc.cluster.local" --upsert`
...
- [x] 1.8.8 Verify ArgoCD Principal deployment
  - Verify Redis pod running: `kubectl get pods -n platform-ops -l app=redis`
  - Verify Principal pod running: `kubectl get pods -n platform-ops -l app=argocd-agent-principal`
  - Verify Principal service exists: `kubectl get svc argocd-agent-principal -n platform-ops`
  - _Requirements: FR-1.2, AC-3_
...
- [x] 1.8.9 Verify ArgoCD Agent connection to Principal
  ...
  - Check Principal logs: `kubectl logs -n platform-ops deployment/argocd-agent-principal | grep spoke-pool-eu-prod-01`
...
- [x] 1.9.5 Create ArgoCD Agent addon templates
  ...
  - ConfigMap namespace: `platform-ops`
...
- [x] 1.9.7 Create ArgoCD Application for cluster BIOS templates
  ...
  - Destination: `platform-ops` namespace
...
- [x] 1.9.9 Verify templates exist in hub cluster
  - Verify Secret exists: `kubectl get secret cilium-addon-template -n platform-ops`
  - Verify Secret exists: `kubectl get secret ccm-addon-template -n platform-ops`
  - Verify ConfigMap exists: `kubectl get configmap argocd-agent-deployment -n platform-ops`
  - Verify ConfigMap exists: `kubectl get configmap argocd-agent-rbac -n platform-ops`
  - _Requirements: FR-1.2, AC-3_
...
- [*] 2.8.6 Verify NATS Leaf Node connection
  ...
  - Verify event received in Hub: `kubectl --context hub exec -n platform-messaging nats-0 -- nats stream info`
  - _Requirements: FR-2.3, AC-4_
...
- [x] 3.2.5.3 Update AINativeSaaS Composition to create dual database users
  ...
  - User A is active initially, User B is inactive (password will be rotated first)
...
- [x] 3.2.5.4 Update AINativeSaaS Composition to generate password Secret
  ...
  - Secret fields: `username: tenant_<id>_user_a`, `password: <random-32-char>`, `database: tenant_<id>_db`, `host: shared-cnpg-rw`, `port: 5432`
