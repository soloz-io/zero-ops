Let me read the rest of the installer.go file to see all the functions:

Perfect! Now I can provide a complete breakdown of what `./bin/hub init-secrets` does:
...
Your CLI is stuck at **Step 3** because:
1. Infisical pods are crashing with "bouncer config error"
2. PgBouncer is missing the `user_search` function (needs `authQuery` in Pooler spec)
3. ArgoCD won't sync the Pooler resource because ExternalSecrets are failing (circular dependency)

The pooler manifest in Git has `authQuery`, but ArgoCD hasn't applied it to the cluster yet. We need to unblock ArgoCD's sync to get the pooler recreated with the correct configuration.