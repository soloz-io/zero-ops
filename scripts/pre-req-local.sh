...
# Zero-Ops Local Provider Prerequisite Checker
# Standalone script. Validates tools needed for local/CAPD development.
# Idempotent: tracks checks in .zero-ops/pre-req-local-state.json.
...
STATE_FILE="$LOG_DIR/pre-req-local-state.json"

mkdir -p "$LOG_DIR"

log() { echo "[$(date '+%Y-%m-%d %H:%M:%S')] $1" | tee -a "$LOG_DIR/pre-req-local.log"; }
...
main() {
    local failed=0
    log "Zero-Ops Local Provider Prerequisite Check"
    ...
    log "" && log "✅ All prerequisites satisfied! Run: bash scripts/hub-bootstrap.sh --provider=local"
}
